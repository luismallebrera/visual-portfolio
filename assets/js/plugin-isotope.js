	...
	VP.prototype.initIsotope = function (options) {
		...
		if (
			...
			const initOptions = options || {
				...
				originLeft: !isRtl,
			// Custom styles for show/hide animations during filtering
			hiddenStyle: {
				opacity: 0,
			},
			visibleStyle: {
...
$doc.on('addItems.vpf', (event, self, $items, removeExisting) => {
	...
	if (removeExisting) {
		...
		// Sequence: hide → position → show
		// Step 1: Hide existing items first
		$existing.css('opacity', '0');
		
		// Step 2: After hide animation completes, remove and add items
		setTimeout(() => {
			self.$items_wrap.isotope('remove', $existing);
			
			// Set initial state for new items (hidden)
			$items.css('opacity', '0');
			...
			// Step 3: Layout items in their new positions (still hidden)
			self.initIsotope('layout');
			
			// Step 4: Show new items after they're positioned
			setTimeout(() => {
				$items.css('opacity', '1');
				self.initIsotope('layout');
			}, 50);
		}, 300); // Wait for hide animation (matches transitionDuration)
	} else {
